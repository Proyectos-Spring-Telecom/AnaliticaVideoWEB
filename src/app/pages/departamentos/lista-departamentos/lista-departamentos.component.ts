...
@Component({
  ...
  styleUrl: './lista-departamentos.component.scss',
  standalone: false,
...
export class ListaDepartamentosComponent implements OnInit {
  ...
  agregarDepartamento() {
    this.router.navigateByUrl('/departamentos/agregar-departamento');
  }

  actualizarDepartamento(idDepartamento: number) {
    this.router.navigateByUrl('/departamentos/editar-departamento/' + idDepartamento);
  }

  activar(rowData: any) {
    Swal.fire({
      title: '¡Activar!',
      html: `¿Está seguro que requiere activar el departamento: <strong>${rowData.nombre}</strong>?`,
      icon: 'warning',
      background: '#141a21',
      color: '#ffffff',
      ...
      if (result.value) {
        this.departamentosService.updateEstatusActivar(rowData.id, 1).subscribe(
          (response) => {
            Swal.fire({
              background: '#141a21',
              color: '#ffffff',
              title: '¡Confirmación Realizada!',
              html: `El departamento ha sido activado.`,
              icon: 'success',
              confirmButtonColor: '#3085d6',
              confirmButtonText: 'Confirmar',
            });
  ...
  desactivar(rowData: any) {
    Swal.fire({
      title: '¡Desactivar!',
      html: `¿Está seguro que requiere desactivar el departamento: <strong>${rowData.nombre}</strong>?`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.value) {
        this.departamentosService.updateEstatusDesactivar(rowData.id, 0).subscribe(
          (response) => {
            Swal.fire({
              title: '¡Confirmación Realizada!',
              html: `El departamento ha sido desactivado.`,
              icon: 'success',
              confirmButtonColor: '#3085d6',
              confirmButtonText: 'Confirmar',
              background: '#141a21',
  ...
  setupDataSource() {
    ...
    this.listaDepartamentos = new CustomStore({
      key: 'id',
      load: async (loadOptions: any) => {
        ...
        try {
          const resp: any = await lastValueFrom(
            this.departamentosService.obtenerDepartamentosData(page, take)
          );
          this.loading = false;
  ...
  onGridOptionChanged(e: any) {
    ...
    if (!texto) {
      this.filtroActivo = '';
      grid?.option('dataSource', this.listaDepartamentos);
      return;
    }
