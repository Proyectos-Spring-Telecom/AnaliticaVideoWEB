...
export class ListaMarcasComponent implements OnInit {
  ...
  agregarMarca() {
    this.router.navigateByUrl('/marcas/agregar-marca');
  }

  actualizarMarca(idMarca: number) {
    this.router.navigateByUrl('/marcas/editar-marca/' + idMarca);
  }
  ...
  activar(rowData: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.marcasService.updateEstatusActivar(rowData.id, 1).subscribe(
          (response) => {
            Swal.fire({
              background: '#141a21',
              color: '#ffffff',
  ...
  desactivar(rowData: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.marcasService.updateEstatusDesactivar(rowData.id, 0).subscribe(
          (response) => {
            Swal.fire({
              title: '¡Confirmación Realizada!',
  ...
  setupDataSource() {
    ...
    this.listaMarcas = new CustomStore({
      key: 'id',
      load: async (loadOptions: any) => {
        ...
        try {
          const resp: any = await lastValueFrom(
            this.marcasService.obtenerMarcasData(page, take)
          );
          this.loading = false;
  ...
  onGridOptionChanged(e: any) {
    ...
    if (!texto) {
      this.filtroActivo = '';
      grid?.option('dataSource', this.listaMarcas);
      return;
    }
