...
export class ListaEquipoComponent implements OnInit {
  ...
  obtenerEquipos() {
    this.equipoService.obtenerEquipos().subscribe((response) => {
      this.listaEquipos = response.data
    })
  }

  agregarEquipo() {
    this.router.navigateByUrl('/equipos/agregar-equipo');
  }

  actualizarEquipo(idEquipo: number) {
    this.router.navigateByUrl('/equipos/editar-equipo/' + idEquipo);
  }
  ...
  activar(rowData: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.equipoService.updateEstatusActivar(rowData.id, 1).subscribe(
          (response) => {
            Swal.fire({
              background: '#141a21',
              color: '#ffffff',
  ...
  desactivar(rowData: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.equipoService.updateEstatusDesactivar(rowData.id, 0).subscribe(
          (response) => {
            Swal.fire({
              title: '¡Confirmación Realizada!',
  ...
  setupDataSource() {
    ...
    this.listaEquipos = new CustomStore({
      key: 'id',
      load: async (loadOptions: any) => {
        ...
        try {
          const resp: any = await lastValueFrom(
            this.equipoService.obtenerEquiposData(page, take)
          );
          this.loading = false;
  ...
  onGridOptionChanged(e: any) {
    ...
    if (!texto) {
      this.filtroActivo = '';
      grid?.option('dataSource', this.listaEquipos);
      return;
    }
