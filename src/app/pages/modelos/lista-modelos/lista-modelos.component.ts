...
export class ListaModelosComponent implements OnInit {
  ...
  obtenerlistaModelos(){
    this.modeloService.obtenerModelos().subscribe((response) => {
      this.listaModelos = response.data
    })
  }

  agregarModelo() {
    this.router.navigateByUrl('/modelos/agregar-modelo');
  }

  actualizarModelo(idModelo: number) {
    this.router.navigateByUrl('/modelos/editar-modelo/' + idModelo);
  }
  ...
  setupDataSource() {
    ...
    this.listaModelos = new CustomStore({
      key: 'id',
      load: async (loadOptions: any) => {
        ...
        try {
          const resp: any = await lastValueFrom(
            this.modeloService.obtenerModelosData(page, take)
          );
          this.loading = false;
  ...
  onGridOptionChanged(e: any) {
    ...
    if (!texto) {
      this.filtroActivo = '';
      grid?.option('dataSource', this.listaModelos);
      return;
    }
